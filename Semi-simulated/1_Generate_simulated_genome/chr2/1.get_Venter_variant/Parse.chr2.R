...
Homo.Indels <- read.table("chr2+.homozygous.indels.gff")
Homo.Indels <- Homo.Indels[Homo.Indels[,1]=="2",]
...
SVs.1000 <- Indels.homo[as.numeric(as.character(Indels.homo[,3]))>1000 & as.numeric(as.character(Indels.homo[,3]))<10000,]
...
Indels.homo <- cbind(Indels.homo,rep("1/1",33436))
...
wrong.pos <- vector()
for (i in 1:45715) # length of indel_all_no_duplicates -1, due the the last position need to be manul check
{
...
write.table(Indel_1,"Chr2.haplotype_1.variants_1.txt",row.names = FALSE,col.names = FALSE,quote = FALSE, sep = "\t")
write.table(Indel_2,"Chr2.haplotype_2.variants_1.txt",row.names = FALSE,col.names = FALSE,quote = FALSE, sep = "\t")
