...
make_plot <- function(P_file,R_file,file_name)
{
  ...
  print_plot <- function(data_plot,y_name,plot_name)
  {
    ...
    p <- p + scale_fill_manual(values=c("#a6cee3","#1f78b4","#b2df8a","#33a02c","#000000"),
                               labels=c("40 - 50 bp","30 - 40 bp","20 - 30 bp"," 10 - 20 bp","1 - 10 bp"))
    p <- p + theme(
    ...
    p <- p + guides(fill=guide_legend(title="Insertions size",reverse = T))
    ...
    #tiff(filename ="legend_short2.tiff",width = 1800,height = 800, pointsize = 17)
    print(p)
    dev.off()
  }
  
  interval_names <- c("40 - 50 bp","30 - 40 bp","20 - 30 bp"," 10 - 20 bp","1 - 10 bp")
...
file_set_genotype <- c("/Deletion_Precision_Genotype_short.txt","/Deletion_Recall_Genotype_short.txt",
                       "/Insertion_Precision_Genotype_short.txt","/Insertion_Recall_Genotype_short.txt")
output_file_genotype <- c("/deletion_Genotypes_short","/insertion_Genotype_short")
file_set <- c("/Deletion_Precision_short.txt","/Deletion_Recall_short.txt",
              "/Insertion_Precision_short.txt","/Insertion_Recall_short.txt")
output_file <- c("/deletion_short","/insertion_short")
